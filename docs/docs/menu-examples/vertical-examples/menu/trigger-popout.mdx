...
export const updatedCode = SourceCode.replace(/customStyles=\{\{([\s\S]*?)\}\}/, '').replace('flex min-bs-full bs-dvh', 'flex').replace(/\s*\/\/\sStyle.*\s*import menuItemStyles.*/, '').replace(/menuItemStyles={.*}/, '').replace(/backgroundColor='.*'/, '')
...
<BrowserWindow url='/vertical-menu/menu/trigger-popout'>
  <Provider>
    <TriggerPopout />
  </Provider>
</BrowserWindow>
