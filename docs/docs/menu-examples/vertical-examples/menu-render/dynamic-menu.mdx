...
export const updatedCode = SourceCode.replace(/customStyles=\{\{([\s\S]*?)\}\}/, '').replace('@docComponents/GenerateMenu', '@compnoents/GenerateMenu').replace('@site/src/types/menuTypes', '@/types/menuTypes')
