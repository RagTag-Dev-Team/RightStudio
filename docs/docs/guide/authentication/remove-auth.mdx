...
- update the `handleSubmit` function in the `src/views/login/LoginForm.tsx` file to the following:

  <Tabs>
  <TabItem value="TS">

  ```tsx title="src/views/login/LoginForm.tsx"
  ...
  ```js title="src/views/login/LoginForm.js"
  ...
  Also, remove the unused hooks and imports from the `src/views/login/LoginForm.tsx` file.
...
- remove the button with the `onClick={() => signIn('google')}` prop from the `src/views/login/LoginForm.tsx` file:
